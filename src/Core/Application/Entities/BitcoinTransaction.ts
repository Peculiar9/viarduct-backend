...
import { IBitcoinTransaction } from "../Interface/Entities/bitcoin/IBitcoinTransaction";

@CompositeIndex(['address', 'tx_hash'])
@CompositeIndex(['wallet_account_id', 'status'])
export class BitcoinTransaction implements IBitcoinTransaction {
    ...
    @ForeignKey({
        ...
        field: '_id',
        constraint: 'fk_bitcoin_transaction_wallet_account_id'
    })
    @Column('UUID')
